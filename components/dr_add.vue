      ...
      <v-layout column>
            ...
            <v-flex xs6 class="mt-2">
              <v-radio-group v-model="imageUrl" row>
                <v-radio
                  label="1"
                  color="primary"
                  value="/img/recipes_lvl1.png"
                />
                <v-radio
                  label="2"
                  color="primary"
                  value="/img/recipes_lvl2.png"
                />
                <v-radio
                  label="3"
                  color="primary"
                  value="/img/recipes_lvl3.png"
                />
                <v-radio
                  label="4"
                  color="primary"
                  value="/img/recipes_lvl4.png"
                />
                <v-radio
                  label="5"
                  color="primary"
                  value="/img/recipes_lvl5.png"
                />
              </v-radio-group>
            </v-flex>
          </v-layout>
        ...
        <v-flex class="add add_ingredients">
          <v-layout column align-center>
            ...
            <v-flex
              v-for="(ingredient, index) in addIngredients"
              :key="index"
            ...
            >
              <v-layout column>
                <v-flex row d-flex align-center>
                  ...
                  />
                  <v-text-field
                    v-model.number="ingredient.portion"
                    label="Количество"
                    solo
                    suffix="гр."
                    type="number"
                ...
                <v-flex row d-flex align-center>
                  ...
                  />
                  <v-text-field
                    v-model.number="ingredient.carb"
                    label="углеводов на 100гр."
                    solo
                    suffix="гр."
                    type="number"
                    :min="0"
          ...
          <v-layout column>
            ...
            >
              <v-layout align-center>
                <v-flex xs10>
                  <v-text-field
                    v-model="addSteps[index]"
                    :label="'Описание шага ' + (index + 1)"
                    required
                    :rules="requireRules"
                    multi-line
                  />
        ...
        <v-flex class="add add_tags">
          <v-layout justify-center row wrap>
            ...
            >
              ...
              <v-select v-model="selectedTags[index]" :items="item.items" />
            </v-flex>
          </v-layout>
        </v-flex>
        <v-flex class="add buttons">
          <v-layout justify-center>
            <v-btn color="primary" block :disabled="!valid" @click="addRecipe">
              Добавить рецепт
            </v-btn>
          </v-layout>
        </v-flex>
  ...
  methods: {
    ...
    async addRecipe() {
      ...
      if (
        ...
        (this.addIngredients.length > 0 &&
          this.addSteps.length > 0 &&
          this.selectedTags.length === 4)
      ) {
        getAllCalFromIngridients()
        getTypeTagCal()
        //await axios.post('https://thesmartestdiet.herokuapp.com/api/administration/add', {
        await axios
          .post("http://127.0.0.1:3000/api/administration/add", {
