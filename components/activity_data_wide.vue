<template>
  <v-layout v-scroll="onScroll" column>
    ...
    >
      <div>
        <v-icon x-large dark class="hidden-xs-only">
          done
        </v-icon>
        <v-icon x-large dark class="hidden-sm-and-up">
          done
        </v-icon>
        <v-btn
          outline
          class="btn-wide-active white"
          :disabled="stateSnackbar !== 'success'"
    ...
    <h3 class="mb-4">
      1. Выберите действия, которые вы выполняли в течение одних суток:
    </h3>
    <v-layout column>
      <v-flex v-for="(select, index) in selects" :key="select.title" column>
        <h3 class="list-titles">
          {{ select.title }}
          <v-tooltip left offset-overflow color="success">
            <v-icon v-if="select.hint" slot="activator" color="success">
              help_outline
            </v-icon>
            <div v-html="select.hint" />
          </v-tooltip>
        </h3>
        <v-select
    ...
    <v-layout id="fieldsList" column>
      <v-layout column>
        <h3 v-if="selectsValue.length > 0" class="my-4">
          2. Введите количество затраченного времени на действия, которые
          выполняли в течение одних суток:
        </h3>
        <v-flex
          v-for="(value, index) in selectsValue"
          :key="index + 'cnt'"
          column
        >
          <v-layout column>
            <v-flex
              v-for="(val, i) in selectsValue[index]"
              :key="val.text"
...
export default {
  ...
  methods: {
    ...
    setTime(value, obj, type) {
      ...
      if (!this.objTime.hasOwnProperty(obj.text))
        this.$set(this.objTime, obj.text, 0)
