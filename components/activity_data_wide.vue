<template>
  <v-layout v-scroll="onScroll" column>
    <v-snackbar vertical auto-height color="primary" :timeout="0" top multi-line v-model="hint" v-if="stateSnackbar === 'normal'">
    ...
    <v-snackbar color="success" :timeout="0" top multi-line v-model="hint" v-else-if="stateSnackbar === 'success'" class="success-notification">
      <div>
    ...
    <v-snackbar vertical color="red" :timeout="0" top multi-line v-model="hint" v-else-if="stateSnackbar === 'error'">
      <h2 class="error-snackbar mb-2">
        <v-icon large dark c>error_outline</v-icon> Ошибка</h2>
    ...
    <v-layout column id="fieldsList">
      <v-layout column>
        <h3 class="my-4" v-if="selectsValue.length > 0">
          2. Введите количество затраченного времени на действия, которые выполняли в течение одних суток:</h3>
        <v-flex column v-for="(value,index) in selectsValue" :key="index + 'cnt'">
          <v-layout column>
            <v-flex v-for="(val,i) in selectsValue[index]" :key="val.text" 
            class="table-v-flex" v-deletelem="{value:0,obj:val,type:'reset',func:setTime}">
              <v-layout row wrap justify-center class="activity-list-container">
                <v-flex xs12 lg3 md3 sm3 class="activity-list-caption">
                  <h4>{{selectsValue[index][i].text}}</h4>
                </v-flex>
                <v-flex xs12 lg4 md4 sm4 class="activity-time mb-3 mr-2">
                  <v-text-field type="number" :min="0" :max="24" id="fieldHours" 
                  solo suffix="ч." @input="setTime($event,val,'hours')">
                  </v-text-field>
                </v-flex>
                <v-flex xs12 lg4 md4 sm4 class="activity-time mb-3 mr-2">
                  <v-text-field type="number" :min="0" :max="60" solo suffix="мин." 
      ...
      return {
        ...
        selects: [{
          ...
          },
          {
            ...
            hint: `
            <span>Если вы не нашли в списках ту активность, котороую вы
             выполняли в течение суток, то вы сами можете приблизительно оценить её.
            </span>
            `,
            items: [{
                text: "Оч. легкая акт.",
